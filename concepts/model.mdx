...
The remote destination, created by `uniac project create <name>` and bound to
your directory by `uniac link` (`.uniac/deploy.json`). The two are separate
acts: linking never creates, and creating never binds. A project is a flat
list of running services; the platform holds no grouping above them.
...
does not remove the service, and the CLI has no removal command. Removal is
done in the dashboard at [uniac.ai](https://uniac.ai): open the project, then
either open a service and choose **Delete service**, or open the project's
Settings and choose **Delete project** — it asks you to type the project's
name, and removes every service and endpoint the project holds.
...
uniac project create   # create the remote project (once per project)
