...
target project, which creates the remote service. Deploying the project
includes the instances from every deployment declaration.
...
Services are addressable as `<instance>.internal` within their project, and
...
that service, and a service keeps its execution type across versions; a
deployment that changes the type is rejected.
