...
[volume](/resources/volume); singleton execution alone does not preserve data.
...
Endpoint addresses use hostnames Uniac issues: the current release has no
custom-domain binding, no per-service certificate and no UDP exposure. An HTTP
request through a public endpoint times out after 60 seconds. A `tcp`
endpoint keeps its allocated public port while the endpoint exists.
...
serving deployment's values. The current release has no separate secret
store; changing a value deploys a new version.
...
whose process exits. It performs no application health or readiness probes.
The current release has no rollback, canary or traffic-splitting controls;
deploying an earlier image creates a new version.
...
activity and resource usage. It sets a stateless service's replica count, up
to four, changes its public endpoints, and offers **Delete service**. Its volume's retention and deletion
are governed by the [volume lifecycle](/resources/volume#lifecycle).
