---
title: "Service"
description: "A service is a named application component within a project. It runs containers from a reusable definition that supplies the image source and runtime configuration. The service keeps its identity across container replacements and deployment versions."
---

{/* Generated from uniac-ai/agent-skills: skills/uniac/references/resources/service/overview.md */}
...
a [volume](/resources/volume) whose data survives container replacement.
...
| Environment variables | No | [Environment values and references](/resources/service/environment). |
| Start command | No | Replaces the image's startup command (`ENTRYPOINT` and `CMD`) at runtime; the image is unchanged. |
| Volume attachment | No | [Durable storage](/resources/volume) for a stateful service. |

A start command is split into arguments with shell-style quoting. A shell is
not automatically invoked.

[Public endpoints](/resources/service/networking) are declared on an instance in a deployment,
