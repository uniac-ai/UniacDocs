...
package: a reference addresses instances from any deployment in its own
package, and the root's resources form their own package. Different packages
may reuse definition names; the concrete services instantiated across the
project have unique names.
...
description, and a definition runs through the declarations that name it.
...
The **service instance** is the named remote service, and its replicas are
the containers running it. Multiple instances can reuse one definition. The
...
included packages. A declaration belongs to the description, and each
resulting service has its own
