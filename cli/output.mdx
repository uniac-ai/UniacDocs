...
description: "deploy prints one final text report to stdout, containing its execution record followed by the state it could report. status prints the state and, if its read fails, an error block. Both reports are text. Help and argument-parsing errors go to stderr and leave stdout empty."
---
...
if its read fails, an error block. Both reports are text; `plan --json` is the
JSON output. Help and argument-parsing errors go to stderr and leave stdout
empty.
...
the start command, and the source definition's name.
...
Each `release` block identifies the service, its package and its deployment
...
| `status` | Reported status, followed by the `(observed/effective)` replica counts. |
...
These two commands use the codes above. Some conditions use a code whose name
...
- [docs.uniac.ai](https://docs.uniac.ai) is the current documentation;
  its [index](https://docs.uniac.ai/llms.txt) lists every page.
- The [Uniac Discord community](https://discord.gg/VR92F2qKjQ) is where
  people ask for help with a deployment.
- support@uniac.ai handles account matters.

Joining the Discord or writing to support is the user's action: a
sign-in or deployment failure that the documentation does not resolve
is reported to the user together with these places.
